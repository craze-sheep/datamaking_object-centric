...
def _get_tf():
  """Lazily import TensorFlow only for remote gfile paths."""
  global _TF
  if _TF is None:
...
def _copy_file(src, dst, overwrite=False):
  dst_str = str(dst)
  if dst_str.startswith("gs://"):
    _get_tf().io.gfile.copy(str(src), dst_str, overwrite=overwrite)
  else:
    dst_path = file_io.as_path(dst)
    dst_path.parent.mkdir(parents=True, exist_ok=True)
    if overwrite:
      dst_path.unlink(missing_ok=True)
    dst_path.write_bytes(file_io.as_path(src).read_bytes())
...
class PyBullet(core.View):
  ...
  def _add_object(self, obj: core.Cylinder) -> Optional[int]:
    radius = obj.scale[0]
    assert radius == obj.scale[1], obj.scale
    height = 2.0 * obj.scale[2]
    collision_idx = self._physics_client.createCollisionShape(
  ...
  def save_state(self, path: Union[pathlib.Path, str] = "scene.bullet"):
    ...
    _copy_file(self.scratch_dir / "scene.bullet", path, overwrite=True)
