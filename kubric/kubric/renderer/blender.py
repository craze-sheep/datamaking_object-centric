...
def _copy_file(src, dst, overwrite=False):
  ...
  if str(dst).startswith("gs://"):
    _get_tf().io.gfile.copy(str(src), str(dst), overwrite=overwrite)
  else:
    if overwrite:
      dst.unlink(missing_ok=True)
    dst.write_bytes(kb.as_path(src).read_bytes())
...
class Blender(core.View):
  ...
  def save_state(self, path: PathLike, pack_textures: bool = True):
    ...
    _copy_file(tmp_path, path, overwrite=True)
  ...
  def _add_asset(self, obj: core.Cylinder):
    ...
    obj.observe(AttributeSetter(cylinder, "active_material",
                                converter=self._convert_to_blender_object), "material")
