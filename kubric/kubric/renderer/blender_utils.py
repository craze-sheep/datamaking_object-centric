...
def _read_exr_with_bpy(filename):
  """Read EXR using Blender's built-in OpenImageIO (no OpenEXR pip needed)."""
...
def read_channels_from_exr(exr, channel_names: Sequence[str]) -> np.ndarray:
...
def _get_render_layers_from_exr_bpy(filename) -> Dict[str, np.ndarray]:
  """Read EXR render layers using Blender's built-in OpenImageIO."""
  ...
  def get_channel(name):
    if name in ch_map:
      return px[:, :, ch_map[name]:ch_map[name]+1]
    return None

  def get_channels(names):
    idxs = [ch_map[n] for n in names if n in ch_map]
    if len(idxs) == len(names):
      return px[:, :, idxs]
    return None

  # Image (RGBA)
  img_rgba = get_channels(["Image.R", "Image.G", "Image.B", "Image.A"])
  if img_rgba is not None:
    output["linear_rgba"] = img_rgba

  # Depth
  depth = get_channel("Depth.V")
  if depth is not None:
    output["depth"] = depth

  # Vector (optical flow)
  flow = get_channels(["Vector.R", "Vector.G", "Vector.B", "Vector.A"])
  if flow is not None:
    output["backward_flow"] = np.stack([flow[..., 1], -flow[..., 0]], axis=-1)
    output["forward_flow"] = np.stack([flow[..., 3], -flow[..., 2]], axis=-1)

  # Normal
  normal = get_channels(["Normal.X", "Normal.Y", "Normal.Z"])
  if normal is not None:
    output["normal"] = normal

  # UV
  uv = get_channels(["UV.X", "UV.Y", "UV.Z"])
  if uv is not None:
    output["uv"] = uv

  # CryptoObject (segmentation)
  crypto_layers = sorted(set(n.rsplit(".", 1)[0] for n in ch_names if n.startswith("CryptoObject")))
  if crypto_layers:
    index_channels = [n + "." + c for n in crypto_layers for c in "RB"]
    alpha_channels = [n + "." + c for n in crypto_layers for c in "GA"]
    idxs = get_channels(index_channels)
    alphas = get_channels(alpha_channels)
    if idxs is not None:
      idxs = idxs.copy()
  ...
  # ObjectCoordinates
  oc = get_channels(["ObjectCoordinates.R", "ObjectCoordinates.G", "ObjectCoordinates.B"])
  if oc is not None:
    output["object_coordinates"] = oc

  return output


def _get_render_layers_from_exr_openexr(filename) -> Dict[str, np.ndarray]:
