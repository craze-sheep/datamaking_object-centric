...
def train(args):
    ...
    if args.resume:
        ...
        if args.mode == 'smoke':
            ...
            num_epochs = args.epochs if args.epochs != 5 else 1
        elif args.mode == 'small':
            max_steps = args.max_steps or 20
            num_epochs = args.epochs if args.epochs != 5 else max(2, resume_ckpt.get('epoch', 0))
        else:
            max_steps = args.max_steps or 50
    ...
    for epoch in range(start_epoch, num_epochs):
        ...
        last_step = min(max_steps, len(train_loader))
        optimizer.zero_grad(set_to_none=True)
